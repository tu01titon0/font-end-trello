...
const SearchUser = (props) => {
    ...
    const handlerAddUser = (val, name) => {
        // eslint-disable-next-line react/prop-types
        if (props.action === "addToBoard") {
            setInput(name)
            // eslint-disable-next-line react/prop-types
            props.setUserId(val);
        } else {
    ...
    return (
        <div style={{position: "relative"}} tabIndex={0} autoFocus={true}>
            ...
            <div className="search-input-wrapper">
                <SearchIcon className="search-input-icons" style={{color: "white"}}/>
                <input
                ...
                {users.length > 0 ? (
                    users.map((item, index) => (
                        <button
                            ...
                            onClick={() => handlerAddUser(item._id, item.userName)}
                            key={index + 1}
                        >
                            <Stack direction={"row"} alignItems={"center"} gap={1} style={{color: "white"}}>
                                <img
                                    src={item.avatarUrl}
                                    alt="profile picture"
                                    height={"20px"}
